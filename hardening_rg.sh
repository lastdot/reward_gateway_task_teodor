...
|                       *** NOTICE TO USERS ***                          |
|                                                                        |
| This computer system is the private property of Reward Gateway      |
...
cat > /etc/motd << 'EOF'
Reward Gateway AUTHORIZED USE ONLY
EOF
...
chown -R root:root /etc/httpd/conf /etc/httpd/bin 
...
echo 'Welcome to Reward Gateway!' > /var/www/html/index.html
